...
class _DenseBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, efficient=False):
        ...
        for i in range(num_layers):
            self.layer.append(nn.ModuleList([_DenseLayer(
                num_input_features,
            ...
            for j in range(i):
                self.layer[i].append(_DenseLayer(
                    growth_rate,
                    growth_rate=growth_rate,
                    bn_size=bn_size,
                    drop_rate=drop_rate,
            ...
            for j in range(i + 1):
                self.add_module('denselayer(%d,%d)' % (i + 1, j), self.layer[i][j])

    def forward(self, feat0):
        feat = [feat0]
        for i in range(self.nl):
            feat.append(self.layer[i][0](feat[0]))
            for j in range(i):
                feat[i + 1] += self.layer[i][j + 1](feat[j + 1])
...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.avg_pool1d(out, kernel_size=3*self.avgpool_size * self.avgpool_size).view(features.size(0), -1)
        #print(out.shape)
